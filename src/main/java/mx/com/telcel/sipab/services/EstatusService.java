package mx.com.telcel.sipab.services;

import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

import lombok.extern.log4j.Log4j2;
import mx.com.telcel.sipab.constant.MessageResponse;
import mx.com.telcel.sipab.models.Estatus;
import mx.com.telcel.sipab.repositories.EstatusRepository;
import mx.com.telcel.sipab.response.Response;

@Log4j2
@Service
public class EstatusService {
	
	@Autowired
	private EstatusRepository repository;

	public Response<List<Estatus>> findAllByOrigen(String origen) {
		return new Response<List<Estatus>>(MessageResponse.SUCCESS_RETRIEVE, repository.findAllByOrigen(origen));
	}
	
}
